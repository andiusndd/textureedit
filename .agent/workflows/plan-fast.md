...
1. **Check for active plan:**
   - If `<WORKING-DIR>/.agent/active-plan` exists AND points to valid directory:
...
2. **Create plan folder** (only if creating new):
   ...
   - Write path to `<WORKING-DIR>/.agent/active-plan`

`<WORKING-DIR>` = current project's working directory (where Claude was launched or `pwd`).

## Workflow
Use `planner` subagent to:
1. If creating new plan: Create directory `plans/YYYYMMDD-HHmm-plan-name` and update `<WORKING-DIR>/.agent/active-plan`.
...
## Related Workflows
- [/plan](./plan.md) - Standard planning
- [/plan-parallel](./plan-parallel.md) - Team-based planning
- [/code](./code.md) - Start implementation
